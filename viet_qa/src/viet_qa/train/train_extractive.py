...
# Modify sys.path safely so that from viet_qa imports work properly locally
...
def prepare_train_features(examples, tokenizer, config):
    # Some examples contains multiple contexts or answers, we unroll them
    tokenized_examples = tokenizer(
        examples["question" if "question" in examples else "question"],
        examples["context" if "context" in examples else "context"],
    ...
    for i, offsets in enumerate(offset_mapping):
        ...
        # If no answers, it's impossible (start/end = cls_index)
        if len(answers) == 0:
